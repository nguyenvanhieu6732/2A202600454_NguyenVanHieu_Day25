...
class ResponseCache:
    ...
    def similarity(a: str, b: str) -> float:
        """Improved similarity using token bigram overlap to better differentiate queries."""
        def get_ngrams(text: str) -> set[str]:
            words = text.lower().split()
            if not words: return set()
            if len(words) == 1: return set(words)
            return set(f"{w1}_{w2}" for w1, w2 in zip(words, words[1:])) | set(words)
        ...
        try:
            ...
            if best_score >= self.similarity_threshold and best_key_match:
                if _looks_like_false_hit(query, best_key_match):
                    self.false_hit_log.append({"query": query, "matched": best_key_match, "score": best_score})
                    return None, best_score
                return best_value, best_score
        except Exception:
